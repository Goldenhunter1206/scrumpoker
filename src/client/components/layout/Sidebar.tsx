      ...
      <SidebarSection title="Session Settings" defaultOpen>
        <div className="space-y-4">
          <div>
            ...
            <div className="mt-1 text-xs text-[#5E6C84]">0, 1, 2, 3, 5, 8, 13, 20, 40, 100, ?</div>
          </div>
          ...
          <div>
            <label className="text-xs font-medium text-[#5E6C84] uppercase tracking-wider">Voting Timer</label>
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className="text-sm font-mono text-[#172B4D] min-w-[3ch] text-center">{votingTimer}</span>
              <button
            ...
            {isFacilitator && (
              <button
                ...
                className="mt-2 w-full h-8 text-xs bg-[#0052CC] text-white rounded-md hover:bg-[#0747A6] transition-colors"
              >
                Start Countdown
              </button>
            )}
          ...
          <div className="space-y-2">
            ...
            {!votingRevealed ? (
              <button
                ...
                className="w-full flex items-center gap-2 px-2 py-2 text-sm text-[#172B4D] hover:bg-[#F4F5F7] rounded-md transition-colors disabled:opacity-40"
              >
                <svg className="w-4 h-4 text-[#5E6C84]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <button
                ...
                className="w-full flex items-center gap-2 px-2 py-2 text-sm text-[#172B4D] hover:bg-[#F4F5F7] rounded-md transition-colors"
              >
                <svg className="w-4 h-4 text-[#5E6C84]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <input
              ...
              className="w-full h-8 px-2 border border-[#DFE1E6] rounded-md text-xs focus:outline-none focus:ring-1 focus:ring-[#0052CC]"
            />
            ...
            <button
              ...
              className="w-full flex items-center gap-2 px-2 py-2 text-sm text-red-700 hover:bg-red-50 rounded-md transition-colors"
            >
        ...
        <div className="space-y-1">
          {participants.map((p) => {
            ...
            return (
              <div
                key={p.name}
                className={`flex items-center gap-2 p-2 rounded-md ${isMe ? 'bg-[#DEEBFF]' : 'hover:bg-[#F4F5F7]'}`}
              >
                <div
                ...
                <div className="flex-1 min-w-0">
                  <div className={`text-sm truncate ${isMe ? 'font-medium text-[#172B4D]' : 'text-[#172B4D]'}`}>
                    {p.name} {isMe && <span className="text-[10px] text-[#0052CC]">(you)</span>}
                  </div>
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  {p.hasVoted && !p.isViewer && (
                    <span className="w-2 h-2 rounded-full bg-[#36B37E]" title="Has voted" />
                  )}
                  <div
                    className={`w-2 h-2 rounded-full ${p.disconnectedAt ? 'bg-gray-400' : 'bg-green-500'}`}
          ...
          {participants.length === 0 && (
            <div className="text-xs text-[#5E6C84] text-center py-2">No participants yet</div>
          )}
        </div>
      </SidebarSection>
    </aside>
